...
protected:
    ...
    // pidOutput reports through an out param, so tests read the value back from here
    float output = 0.0f;
};

TEST_F(PIDTest, ProportionalControl) {
    PID pid(KP, 0.0f, 0.0f, TAU, OUTPUT_MIN, OUTPUT_MAX, INTEGRAL_MAX_PCT, DT);
    EXPECT_EQ(pid.pidInitState(), ZP_ERROR_OK);

    EXPECT_EQ(pid.pidOutput(10.0f, 0.0f, output), ZP_ERROR_OK);
...
TEST_F(PIDTest, IntegralAccumulation) {
    ...
    float error = 10.0f;
    EXPECT_EQ(pid.pidOutput(error, 0.0f, output), ZP_ERROR_OK);
    EXPECT_EQ(pid.pidOutput(error, 0.0f, output), ZP_ERROR_OK);
...
TEST_F(PIDTest, IntegralWindupClamping) {
    ...
    for (int i = 0; i < 100; i++) {
        EXPECT_EQ(pid.pidOutput(100.0f, 0.0f, output), ZP_ERROR_OK);
    }
...
TEST_F(PIDTest, DerivativeResponse) {
    ...
    EXPECT_EQ(pid.pidOutput(0.0f, 0.0f, output), ZP_ERROR_OK);
    EXPECT_EQ(pid.pidOutput(0.0f, 10.0f, output), ZP_ERROR_OK);
