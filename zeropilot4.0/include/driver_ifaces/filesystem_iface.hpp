...
// NOTE: for functions like inits that do not happen in a thread, can use the SYNC option to force the operation, functions that don't take options are always syncronous

        virtual FileStatus open (ManId id, File* fp, const char* path, const char* mode) = 0;			/* Open or create a file */
        virtual FileStatus close (ManId id, File* fp) = 0;											    /* Close an open file object */
        virtual FileStatus read (ManId id, File* fp, void* buff, uint32_t btr, uint32_t* br) = 0;			/* Read data from the file */
        ...
        virtual FileStatus seek_and_write (ManId id, File* fp, const void* buff, uint32_t btw, uint32_t* bw, uint64_t ofs) = 0;	/* Seek and write data to the file */
        virtual FileStatus write_and_sync (ManId id, File* fp, const void* buff, uint32_t btw, uint32_t* bw) = 0;	/* Write data to the file and sync immediately (for critical logs) */
        ...
        virtual FileStatus mkdir (ManId id, const char* path) = 0;								        /* Create a sub directory */
        virtual FileStatus stat (ManId id, const char* path, FileInfo* fno) = 0;					        /* Get file status */

        virtual int printf (ManId id, File* fp, const char* str, ...) = 0;						        /* Put a formatted string to the file */
