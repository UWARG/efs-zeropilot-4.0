...
ZP_ERROR_e PowerModule::init() {
    ...
    if (!success) result = ZP_ERROR_NOT_READY; 
    uint8_t pData[2];
    ...
    */
    ...
    if (!success) result = ZP_ERROR_FAIL; 
...
ZP_ERROR_e PowerModule::parse(I2C_HandleTypeDef *hi2c) {
    //start the cycle
    if (dataFilled) return ZP_ERROR_FAIL;
    ZP_ERROR_e result = readRegister(REG_VBUS.address, vbusData, REG_VBUS.byte_size, hi2c);
    return result;
}
