...
ZP_Error Rangefinder::init() {
    ...
    ZP_Error status = sendCmdCheckResp(FIRMWARE_VERSION_CMD, sizeof(FIRMWARE_VERSION_CMD),
    ...
    status = sendCmdCheckResp(OUTPUT_FORMAT_CM_CMD, sizeof(OUTPUT_FORMAT_CM_CMD),
    ...
    // Save configs
    return sendCmdCheckResp(SAVE_CONFIG_CMD, sizeof(SAVE_CONFIG_CMD),
                            SAVE_CONFIG_SUCCESS_RESPONSE, sizeof(SAVE_CONFIG_SUCCESS_RESPONSE));
}

ZP_Error Rangefinder::readData(RangefinderData_t &outData) {
    ...
    if (rxBuffer[0] == DATA_FRAME_HEADER && rxBuffer[1] == DATA_FRAME_HEADER) { // Check the frame header so we don't parse a corrupted frame
        ...
        */
        if (computeChecksum() != rxBuffer[8]) {
            result |= ZP_ERROR_CRC;
        }
        if (rawStrength == STRENGTH_SATURATED || rawDistance == DIST_SATURATED || rawDistance == DIST_WEAK_SIGNAL) {
            result |= ZP_ERROR_INVALID_DATA;
        }

        data.isValid = (result == ZP_ERROR_OK);
        data.isNew = true;
...
ZP_Error Rangefinder::restartTransfer() {
    ...
    return halToZpError(HAL_I2C_Master_Transmit_IT(hi2c, TF02PRO_I2C_ADDR, (uint8_t*)I2C_READ_CMD, READ_CMD_LEN));
}
...
ZP_Error Rangefinder::writeDataBlocking(uint8_t* cmd, uint16_t size, uint32_t delay) {
    return halToZpError(HAL_I2C_Master_Transmit(hi2c, TF02PRO_I2C_ADDR, cmd, size, delay));
}

ZP_Error Rangefinder::readDataBlocking(uint8_t* receiveBuffer, uint16_t size, uint32_t delay) {
    return halToZpError(HAL_I2C_Master_Receive(hi2c, TF02PRO_I2C_ADDR, receiveBuffer, size, delay));
}

ZP_Error Rangefinder::sendCmdCheckResp(const uint8_t *cmd, uint16_t cmdSize,
                                       const uint8_t *expectedResp, uint16_t expectedRespSize) {
    uint8_t receiveBuffer[MAX_CMD_RESPONSE_LENGTH] = {0};

    if (expectedRespSize > MAX_CMD_RESPONSE_LENGTH) {
        return ZP_ERROR_RANGE;
    }

    ZP_Error status = writeDataBlocking((uint8_t*)cmd, cmdSize, HAL_MAX_DELAY);
    if (status != ZP_ERROR_OK) {
        return status;
    }
    ...
    for (uint8_t i = 0; i < expectedRespSize; i++) {
        if (receiveBuffer[i] != expectedResp[i]) {
            return ZP_ERROR_CRC; // Device responded but does not match the expected response
        }
    }
    return ZP_ERROR_OK;
}
