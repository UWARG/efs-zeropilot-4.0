...
usb_uart* usb_uart::instance = nullptr;

usb_uart::usb_uart(UART_HandleTypeDef* huart) : huart(huart), readIndex(0), writeIndex(0) {
    instance = this;
}
...
void usb_uart::transmit(const uint8_t* data, uint16_t size) {
    if (huart) {
        HAL_UART_Transmit_DMA(huart, data, size);
    }
}

uint16_t usb_uart::getRXTransferSize(uint16_t idx) {
	if (idx > lastIdx) {
...
void usb_uart::receiveCallback(uint16_t writeIdx) {
...
uint16_t usb_uart::receive(uint8_t* buffer, uint16_t bufferSize) {
...
UART_HandleTypeDef* usb_uart::getHUART() const {
    return huart;
}
